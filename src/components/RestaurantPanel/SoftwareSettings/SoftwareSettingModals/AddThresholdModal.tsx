...
const AddThresholdModal: React.FC<Props> = ({
  onClose,
  thresholdId,
  setLoading,
}) => {
  ...
  const [onlineOrderingVisibilityStatus, setOnlineOrderingVisibilityStatus] =
    useState(true);
  ...
  const validateForm = (): boolean => {
    ...
    if (!thresholdAmount)
      newErrors.thresholdAmount = "Threshold amont is required";
    if (
      !discountValue ||
    ...
    if (
      !transactionLimit ||
      isNaN(Number(transactionLimit)) ||
      Number(transactionLimit) <= 0
    )
      newErrors.transactionLimit =
        "Transaction limit must be a positive number";
  ...
  const handleSubmit = async () => {
    if (validateForm()) {
      ...
      }api/add/update/threshold?restaurantLoginId=${RestaurantLoginId_Global}`;
      try {
        setLoading(true);
  ...
  const handleThresholdAmountChange = (
    e: React.ChangeEvent<HTMLInputElement>
  ) => setThresholdAmount(e.target.value);
  const handleDiscountValueChange = (e: React.ChangeEvent<HTMLInputElement>) =>
    setDiscountValue(e.target.value);
  const handleTransactionLimitChange = (
    e: React.ChangeEvent<HTMLInputElement>
  ...
  const handleVisibilityChange = (platform: string, checked: boolean) => {
    ...
    if (platform === "Online Ordering")
      setOnlineOrderingVisibilityStatus(checked);
  };
  ...
  const getThresholdDataById = async (Id: number) => {
    ...
    const apiUrl = `${
      import.meta.env.VITE_API_URL
    }api/get/single/threshold/data?Id=${Id}&restaurantLoginId=${RestaurantLoginId_Global}`;
    try {
      ...
      if (response.status === 200 && response.data.status === 1) {
        ...
        if (
          thresholdData &&
          thresholdData.ThresholdTimings &&
          thresholdData.ThresholdTimings.length > 0
        ) {
          const firstTiming = thresholdData.ThresholdTimings[0];
          setTimingId(Number(firstTiming.Id));
          const fromTime24 = convertTo24HourFormat(
            firstTiming.FromTime_12HoursFormat
          );
          const toTime24 = convertTo24HourFormat(
            firstTiming.ToTime_12HoursFormat
          );
        ...
        setOnlineOrderingVisibilityStatus(
          thresholdData.OnlineOrderingVisibilityStatus === 1
        );
      }
    } catch (error) {
      if (axios.isAxiosError(error)) {
  ...
  return (
    <div
      className="modal fade advance-btn-modal show !pl-0"
    ...
    >
      <div className="modal-dialog modal-lg modal-dialog-centered">
        <div className="modal-content w-100 w-100 max-h-[550px] sm:max-h-full">
          <div className="modal-header pb-0">
            <h5 className="modal-title mb-2" id="thresholdModalLongTitle">
              {thresholdId === 0 ? "Add New" : "Edit"} Threshold
            </h5>
          </div>
          <div className="modal-body pt-1 overflow-y-auto">
              ...
              <div className="row">
                        ...
                        <div className="row">
                          <div className="col-6 ps-0 pe-1">
                            <div className="input-container">
                              ...
                              <input
                                type="date"
                                id="thresholdDateFrom"
                                value={dateFrom}
                                onChange={(e) => setDateFrom(e.target.value)}
                              />
                              {errors.dateFrom && (
                                <span className="errorsClass2 text-red-600 !text-xs">
                                  {errors.dateFrom}
                                </span>
                              )}
                            </div>
                ...
                <div className="col-12 col-md-6">
                  {thresholdTimingTypeId === "2" && (
                    <form
                      className="timing-form mb-1"
                      id="threshold_TimingSection"
                    >
                        ...
                        {[
                          ...
                          <li key={day}>
                            <div>
                              ...
                              />
                              <label htmlFor={`thresholdTiming${day}`}>
                                {day.slice(0, 3)}
                              </label>
                            </div>
                          </li>
                        ...
                        <div className="toggle-btn me-4">
                          <label className="switch mb-0">
                            <input
                              ...
                              onChange={(e) =>
                                handleVisibilityChange(label, e.target.checked)
                              }
                            />
                ...
                <div className="col-12 mt-2">
                  <div className="form-check form-check-inline !inline-flex">
                    ...
                    <label
                      ...
                      htmlFor="thresholdTimingTypeContinuous"
                    >
                      Continuous
                    </label>
                  </div>
                  <div className="form-check form-check-inline !inline-flex">
                    ...
                    <label
                      className="form-check-label"
                      htmlFor="thresholdTimingTypeSpecific"
                    >
          ...
          <div className="modal-footer">
            ...
            <button type="button" className="pro-submit" onClick={handleSubmit}>
              Submit
            </button>
          </div>
        </div>
