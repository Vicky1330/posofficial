...
const AddProductForm: React.FC<Props> = ({ setLoading, loading }) => {
  ...
  const getProductById = async (productId: number) => {
    ...
    try {
      ...
      if (response.status === 200 && response.data.status === 1) {
        ...
        data.ProductBarcodeImage === ""
          ? setBarcodeImageUrl(null)
          : setBarcodeImageUrl(
            `http://posofficialnew.protoshopp.in/${data.ProductBarcodeImage}`
          );
  ...
  const updateProductImage = async (Id: number) => {
    ...
    try {
      await axios.post(
        `${import.meta.env.VITE_API_URL}api/product/upload/image`,
        data,
        {
          headers: {
            Authorization: `Bearer ${UserToken_Global}`,
  ...
  const handleInsertUpdateProduct = async () => {
    ...
    formData.append("Recommended", String(productData.recommended));
    formData.append("FeaturedProduct", String(productData.featuredProduct));
    formData.append("ForceSelling", String(productData.forceSelling));
    try {
      // setLoading(true);
      ...
      else if (response?.data?.status === 2 && response?.status === 200) {
  ...
  useEffect(() => {
    ...
    fetchMainDepartmentList();
  }, [loading, mainDepartment, productId]);
  ...
  return (
    <>
      <div className="tab-pane active" id="AddUpdateProduct_tab">
        ...
        <div
          className="main_deapt !p-0 py-3 sm:py-0 sm:!p-5 "
          ...
          <form className="new_customer-wrap">
                    ...
                    <div className="checkform !p-0 sm:pl-2">
                      <div className="grid grid-cols-3">
                        <label className="switch round_wraps product_option_style">
                          <input
                            type="checkbox"
                            checked={productData.recommended === "1"}
                            onChange={(e) =>
                              setProductData((prevData) => ({
                                ...prevData,
                      ...
                      <div className="grid grid-cols-3">
                        <label className="switch round_wraps product_option_style">
                          <input
                            type="checkbox"
                            checked={productData.featuredProduct === "1"}
                            onChange={(e) =>
                              setProductData((prevData) => ({
                                ...prevData,
                                featuredProduct: e.target.checked ? "1" : "0",
                              }))
                            }
                          />
                          <span
                      ...
                      <div className="grid grid-cols-3">
                        <label className="switch round_wraps product_option_style">
                          <input
                            type="checkbox"
                            checked={productData.forceSelling === "1"}
                            onChange={(e) =>
                              setProductData((prevData) => ({
                                ...prevData,
                                forceSelling: e.target.checked ? "1" : "0",
                              }))
                            }
                          />
                          <span
            ...
            <div className="col-12">
              <div className="text-end flex">
                <Link to="/Restaurant/ManageProducts/">
                  <button
                    type="button"
      ...
      {isSubDepartmentModal && (
        <div
          className="modal advance-btn-modal"
        ...
        >
          <div className="modal-dialog cstm_modal_dialog">
            <div className="modal-content plus_modal_cont">
              {/* Modal Header */}
              <div className="modal-header plus_modal_head mx-auto">
                <h4 id="heading_Title_SubDepartmentModal" className=" ">
                  Add Department
                </h4>
              </div>
